    ...
    const soulEnforceInterval = setInterval(() => {
      try {
        ...
        if (!current.includes(marker)) {
          let soulLines = '';
          const repoSoul = '/app/SOUL.md';
          if (existsSync(repoSoul)) {
            try {
              soulLines = readFileSync(repoSoul, 'utf8');
              console.log('[soul-enforcer] Using SOUL.md from repository source');
            } catch (e) {
              console.warn('[soul-enforcer] Failed to read /app/SOUL.md:', e.message);
            }
          }

          // Fallback if /app/SOUL.md is missing or failed to read
          if (!soulLines) {
            soulLines = [
              '# Soul',
              '',
            ...
            console.log('[soul-enforcer] Using hardcoded fallback for SOUL.md');
          }

          mkdirSync(wsDir, { recursive: true });
          writeFileSync(soulFile, soulLines, 'utf8');
          console.log('[soul-enforcer] SOUL.md re-written');
        }
      } catch (e) {
...
async function runPostStartupTasks(configFile, context = '') {
  ...
  try {
    ...
    if (existsSync(repoSoul)) {
      try {
        soulContent = readFileSync(repoSoul, 'utf8');
        console.log(`Using SOUL.md from repository source${logSuffix}`);
      } catch (e) {
        console.warn(`Failed to read /app/SOUL.md${logSuffix}:`, e.message);
      }
    }
    ...
    if (!soulContent) {
      ...
      console.log(`Using hardcoded fallback for SOUL.md${logSuffix}`);
    }

    writeFileSync(soulMdPath, soulContent, 'utf8');
    console.log(`SOUL.md overwritten${logSuffix}.`);
  } catch (soulErr) {
